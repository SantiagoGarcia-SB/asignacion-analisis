...
| `SAI_INCLUIR_BORRADOR` / `SAI_INCLUIR_EN_ESTUDIO` | Whether `_sincronizarVentanaSAI()` (Código.js, called by `actualizarSolicitudesNuevasAPI`) lets solicitudes SAI reports as `BORRADOR`/`EN_ESTUDIO` into `solicitud`. Independent booleans, default `true` (include — historical behavior) unless explicitly set to the string `"false"`. Admin-configurable via `admin_getConfigSincronizacionSAI()`/`admin_setConfigSincronizacionSAI()` (Admin.js), exposed as the "Sincronización SAI" button in `VistaAdmin.html`'s config strip. Deliberately kept out of `ESTADOS_EXCLUIR` (which also drives `eliminarSolicitudesFinalizadas`) — these two only gate *new* inserts, never delete existing rows. Does not touch `APROBADO_PENDIENTE_BIOMETRIA` handling, which returns from a separate branch earlier in the same function. |
...
### Motivos Catalog (aplazamiento/negación reasons)

`MotivosAplazamiento`/`MotivosNegacion` (sheets in the main spreadsheet, columns `id | motivo | activo | orden | aplica_digital | aplica_reestudio | aplica_biometria`) are **the single shared catalog** for the "motivo de aplazamiento"/"motivo de negación" dropdowns across all modals — Digital/Cánones Altos (`dig_*`), Reestudios/UAR (`rst_*`), and Desaplazamiento/Biometría (`bio_*`). Before 2026-07-24, Biometría's modal had its own hardcoded `<option>` list in `VistaUnificada.html` (6 fixed aplazamiento reasons, and a single negación reason — "Desistimiento" — auto-selected in `bioActualizarMotivo()`), with no admin CRUD; it has since been migrated into this same catalog automatically by `_getHojaMotivos()`/`_asegurarEsquemaScopeMotivos()` (see "Manual only" below for the optional confirmation trigger, `admin_migrarEsquemaMotivosScope()`).

Managed via `Admin.js`'s `_getHojaMotivos`/`_getMotivos`/`admin_guardarMotivo`/`admin_eliminarMotivo`, and edited from `VistaAdmin.html`'s "Catálogo de Configuración" → Motivos Aplazamiento/Negación tabs. Each motivo carries 3 independent boolean scope flags (`aplicaDigital`/`aplicaReestudio`/`aplicaBiometria`, one or more can be true) controlling which modal's dropdown it appears in — set via checkboxes in the admin's motivo modal, not mutually exclusive (e.g. "Desistimiento" applies to both the shared Digital/Reestudio dropdown and Biometría's). `getMotivosAplazamiento()`/`getMotivosNegacion()` (the public, non-admin getters) still return the full active list in one call each — no `modalTipo` parameter — and `VistaUnificada.html`'s `cargarMotivosUnificados()` filters client-side per select group (`dig_*`/`rst_*`/`bio_*`) from that single response, to avoid adding extra `google.script.run` round-trips. There is no server-side validation of the saved `motivo_aplazamiento`/`motivo_negacion` text against this catalog — `guardarCambiosInternos`, `guardarGestionBiometria`, and `guardarGestionReestudio` persist whatever string the client sends.
...
- `cicloBiometriaPendiente()` — 8am and 12pm: calls `limpiarBiometriasResueltas()`, `_archivarColaBiometriaVencida()`, and `_procesarCortePendientes()` in sequence (refresh, archive expired, escalate WA_ENVIADO cases to the call queue). Since 2026-07-14, the whole function returns immediately (does none of the three) if `_esDiaNoHabilOperacion(new Date())` is true (Sunday or a date in the `Festivos` sheet) — before this guard, the function ran unconditionally every day including Sundays/holidays, so `_archivarColaBiometriaVencida()` could archive a case purely on wall-clock hours elapsed even on a day operación never worked to call it, recreating the same kind of premature-archival problem the `fecha_actualizacion_fase` fix (below) addressed from a different angle. `_esDiaNoHabilOperacion()` was extracted out of `_dentroDeVentanaLey2300()` (same Sunday/`Festivos` check, minus the hour-of-day part) so both guards share one holiday source of truth. `desaplazamiento`/`biometriaFallida` cases only ever become assignable (both to analysts via `RequestLeadUnificado` and via `autoAsignarBiometria()`) once `_procesarCortePendientes()` escalates them into `solicitud` — there's no path that skips the WA→escalation cycle, so a Ley 2300/holiday-blocked stretch (see `_dentroDeVentanaLey2300()`) with no new WA sends means no new escalations either, and the assignable queue can run dry until sending resumes. Since 2026-07-17, `_archivarColaBiometriaVencida()` ages a case from SAI's `fechaResultado`/last-movement date (refreshed every cycle by `limpiarBiometriasResueltas()`, which always runs first in `cicloBiometriaPendiente()`), falling back to `fechaRadicacion` and, only if no SAI date is parseable at all, to the timestamp the case actually entered the queue (`pendiente_biometria`'s `fecha_actualizacion_fase` at the moment its fase became `ESCALADA`). This supersedes the 2026-07-14 approach (queue-entry date as primary, SAI dates as fallback) — confirmed with operación that call-center agents themselves decide whether a case is still worth calling based on that same SAI date, so the automated archival trigger has to match that criterion; otherwise the system could keep offering cases operación would no longer consider callable, or archive ones they still would call. The accepted tradeoff: a case delayed for days by a blocked Ley 2300/holiday WA window can still carry an old SAI date once it finally escalates, and may get archived almost immediately instead of getting its full ~12h in queue — which is fine under this criterion, since operación wouldn't call it either. There's still no automatic un-archive; `admin_desarchivarBiometrias()` (see "Manual only" below) is the manual recovery path. `_procesarCortePendientes()` caps itself at `MAX_CANDIDATOS_POR_CORTE` (500, oldest-`fecha_actualizacion_fase`-first) and writes in blocks of `TAMANO_BLOQUE_ESCRITURA_CORTE` (50) as it goes, with a `TIEMPO_MAXIMO_CONSULTAS_CORTE_MS` (20 min) time backstop — before 2026-07-14 it queried SAI for every WA_ENVIADO candidate first and wrote nothing until the whole batch finished, so a backlog large enough to exceed Apps Script's execution limit (confirmed real case: 1369 candidates, still running past 23 minutes) got killed before a single write happened, and the next corte re-queried the identical backlog from scratch forever. `_enviarPrimerContactoBiometria()` and `limpiarBiometriasResueltas()` still have this same query-all-then-write-all shape and are exposed to the same risk if their own candidate counts grow large enough — not yet fixed. Since 2026-07-22, `_procesarCortePendientes()`'s internal `_volcarBloque()` calls `procesarYGuardarLote()` (the write into `solicitud`) **first**, wrapped in `try/catch`, and only marks a case's `pendiente_biometria` row as `ESCALADA` if that call succeeded — before this fix, the order was reversed (`ESCALADA` was flushed to `pendiente_biometria` *before* calling `procesarYGuardarLote()`, with no `try/catch` around it), so a failure in the `solicitud` write left the case permanently marked `ESCALADA` while never actually reaching `solicitud`. No automatic cycle ever revisits a row already in `ESCALADA` (only `WA_ENVIADO` rows are retried by the next corte), so such a case was invisible forever — to the assignment engine, to `_archivarColaBiometriaVencida()` (which only scans `solicitud`), and to every recovery tool except a from-scratch manual investigation. Confirmed real incident, 2026-07-22: solicitud 12269234 plus a batch of 29 IDs from the same 12pm corte block — all 29 had, by the time they were found (~11-12h later), independently resolved in SAI (26 `APROBADO`, 3 `RECHAZADO`) without ever being called, so no customer impact that time, but the failure mode was real. Cases that fail the `solicitud` write now simply stay `WA_ENVIADO` and get retried by the next corte automatically. `diagnosticarHuerfanasBiometriaEscalada()` (see "Manual only" below) is a read-only reconciliation check added as a second line of defense, independent of this fix.
...
**Manual only (not triggers — run from the Apps Script editor on demand):** `diagnosticarDestinosBiometria()` / `diagnosticarDestinosBiometriaTest()` (debug a single case's WhatsApp destinations), `corregirBiometriasMalEnrutadas()` (idempotent one-off repair for misrouted cases), `forzarPrimerContactoBiometriaManual()` (force first-contact WhatsApp without waiting for the 4h window), `diagnosticarSolicitudCodeudor()` / `diagnosticarSolicitudCodeudorTest()` (`Código.js` — read-only, checks all 4 codeudor-flow locations plus SAI's live status for one solicitud id), `recuperarSolicitudCodeudorManual()` / `recuperarSolicitudCodeudorManualTest()` (`Código.js` — moves one already-identified stuck solicitud into `pendiente_codeudor` immediately; matters more now that `sincronizarHistoricoSAI()` is suspended and its rotation isn't running at all), `admin_listarBiometriasArchivadas()` / `admin_desarchivarBiometrias(cantidad)` (`Biometria.js` — also exposed as a "Desarchivar Biometrías" button in `VistaAdmin.html`'s dashboard config strip; recovers the N most recently `ARCHIVADA` cases from `pendiente_biometria`, revalidating each against SAI before reinserting into `solicitud`'s call queue — see note below), `diagnosticarArchivadoColaBiometria()` (`Biometria.js` — read-only, replays `_archivarColaBiometriaVencida()`'s Phase 1 candidate selection and current threshold without archiving anything, logging which pending cases would/wouldn't be archived right now and why — added 2026-07-14 to let an admin sanity-check the archival timing fix before/after a Ley 2300-blocked stretch), `diagnosticarLiberacionDesaplazamiento()` (`Biometria.js` — read-only, replays the same candidate selection as `RequestLeadUnificado`/`autoAsignarBiometria` against `_calcularLimiteLiberacionDesaplazamiento()` without assigning anything, logging which pending desaplazamiento cases are currently offerable to an analyst vs. still waiting for their release window — added 2026-07-14 alongside the release-window gating itself), `diagnosticarHuerfanasBiometriaEscalada()` (`Biometria.js` — read-only, added 2026-07-22 alongside the `_volcarBloque()` ordering fix above as an independent reconciliation check: for every `pendiente_biometria` row in fase `ESCALADA`, confirms the case actually exists in `solicitud` or `Historico_Gestiones`; reports any that are in neither — the orphan pattern from the 12269234 incident — without modifying anything), `recuperarLoteHuerfanasBiometria()` / `cerrarHuerfanasBiometriaResueltas()` (`Biometria.js` — one-off incident repair from 2026-07-22, hardcoded to the 29 IDs from that batch: the first re-inserts into `solicitud` any that SAI still reports pending; the second closes out (`fase="RESUELTA"`) any that SAI already resolved on its own. Both already run for that incident; kept as a reference pattern for `diagnosticarHuerfanasBiometriaEscalada()` if it ever finds a new batch, not meant for reuse as-is since the ID list is hardcoded), `admin_migrarEsquemaMotivosScope()` (`Admin.js` — optional manual trigger, added 2026-07-24 alongside the motivos scope feature below: the actual migration — adding the `aplica_digital`/`aplica_reestudio`/`aplica_biometria` columns to `MotivosAplazamiento`/`MotivosNegacion` if missing, backfilling existing rows as `TRUE,TRUE,FALSE` (preserving the prior Digital+Reestudio-shared behavior), marking the existing `"Desistimiento"` row as also `aplica_biometria=TRUE`, and appending the 6 aplazamiento motivos that used to be hardcoded in Biometría's modal, scoped `aplica_biometria=TRUE` only — is **automatic**: `_getHojaMotivos()`/`_asegurarEsquemaScopeMotivos()` run it lazily and idempotently the first time any call touches either sheet after this column doesn't yet exist, so no manual step is required after deploying this change and no dropdown is ever left empty waiting on it. This function just lets an admin trigger/confirm it explicitly without waiting for organic traffic).
...
- **Concurrency:** Always use `LockService.getScriptLock()` with `waitLock(...)` before writing to any *shared* state, and always release it in a `finally` block (or `if (lock.hasLock()) lock.releaseLock()` inside `finally`) — never release manually before each `return`, since an exception thrown between `waitLock()` and a manual release leaves the lock held until it expires, blocking every analyst until then. `ScriptLock` is global to the whole script project, not scoped to a sheet or row — there is no per-row lock in Apps Script.
  - **Exception (since 2026-07-22):** the 3 "guardar gestión" endpoints — `guardarCambiosInternos` (Código.js), `guardarGestionBiometria` (Biometria.js), and the `HISTORICO` route of `guardarGestionReestudio` (Reestudios.js) — deliberately do **not** wrap their `Historico_Gestiones` row search/write in `ScriptLock`. Each solicitud lives in its own row, and Sheets safely handles concurrent writes to disjoint rows; taking the global lock for that write only serialized every analyst's save behind everyone else's (was the cause of a real production slowdown/queueing complaint). Each of these revalidates that the located row still matches the expected `solicitudId` immediately before writing (in case an admin op like `desasignarSolicitud`/`admin_reasignarSolicitud` shifted rows via `deleteRow` in between), then calls the shared helper `_cerrarConteoConLockCorto()` (Código.js) — this is the one part of the save that *does* need `ScriptLock`, since it wraps `_registrarCierreContador()`'s read-modify-write of the shared cupo/carga-pendiente counters in `PropertiesService`. If that short lock can't be acquired, the row write already succeeded and is NOT rolled back — only the counter update is skipped, logged, and left for `trigger_recalcularContadores()`'s nightly recalculation to fix (the same recovery path already accepted for admin-caused counter drift). The legacy `ORIGEN` route inside `guardarGestionReestudio` still takes `ScriptLock` around its write, because it also does `deleteRow()` there, which (unlike a plain `setValue`) shifts every subsequent row's index in that sheet and so must stay serialized.
